const days = [
  {
    "date": "3.26",
    "dow": "금",
    "city": "밀라노",
    "title": "Ciao, Italia",
    "sub": "도착하는 날은 가볍게",
    "stay": "밀라노 · 중앙역 또는 브레라 인근 / 2박 중 1박",
    "move": "공항 → 시내 약 1~1.5시간",
    "note": "KE927 · ICN 12:15 출발은 한국 시간. MXP 도착 시각은 항공권 확인 후 아래 일정을 조정하세요. 이날은 입장 예약을 잡지 않습니다.",
    "spots": [
      {
        "time": "도착 후",
        "name": "밀라노 말펜사 공항",
        "lat": 45.6301,
        "lng": 8.7231,
        "kind": "공항",
        "desc": "입국·수하물 수령에 1~2시간 여유를 두고 시내로 이동. Malpensa Express 또는 택시를 이용합니다.",
        "tip": "터미널과 운행 시간은 항공권·공식 교통편에서 확인.",
        "query": "밀라노 말펜사 공항"
      },
      {
        "time": "체크인 후",
        "name": "브레라 지구",
        "lat": 45.4719,
        "lng": 9.1876,
        "kind": "산책",
        "desc": "호텔에 짐을 놓고 골목을 천천히 걷기. 첫 저녁은 리소토 알라 밀라네제와 가벼운 식사.",
        "tip": "피곤하면 산책을 생략하고 호텔 근처에서 식사.",
        "query": "브레라 지구"
      }
    ]
  },
  {
    "date": "3.27",
    "dow": "토",
    "city": "밀라노",
    "title": "지붕 위에서 시작하는 여행",
    "sub": "두오모와 브레라, 운하의 저녁",
    "stay": "밀라노 / 2박 중 2박",
    "move": "도보 + 지하철 · 렌터카 없이",
    "note": "두오모는 오전 예약을 추천. 최후의 만찬은 취향에 따라 브레라 산책 대신 넣되, 예매 성공 후 확정하세요.",
    "spots": [
      {
        "time": "09:00–11:00",
        "name": "밀라노 두오모",
        "lat": 45.4642,
        "lng": 9.1916,
        "kind": "예약",
        "desc": "대성당과 루프톱을 함께 관람. 첨탑 사이에서 둘만의 사진을 남기세요.",
        "tip": "공식 사이트에서 입장 시간 예약. 예배·행사에 따라 관람 변경 가능.",
        "query": "밀라노 두오모"
      },
      {
        "time": "11:00–12:00",
        "name": "비토리오 에마누엘레 2세 갤러리아",
        "lat": 45.4658,
        "lng": 9.19,
        "kind": "산책",
        "desc": "아케이드 구경과 커피. 점심은 주변에서 코톨레타 또는 파스타.",
        "tip": "",
        "query": "비토리오 에마누엘레 2세 갤러리아"
      },
      {
        "time": "14:00–16:00",
        "name": "브레라 미술관 일대",
        "lat": 45.4719,
        "lng": 9.1881,
        "kind": "산책",
        "desc": "미술관 관람 또는 디자인 상점과 골목 산책 중 컨디션에 맞춰 선택.",
        "tip": "",
        "query": "브레라 미술관 일대"
      },
      {
        "time": "18:00–20:00",
        "name": "나빌리오 그란데",
        "lat": 45.4505,
        "lng": 9.1706,
        "kind": "식사",
        "desc": "운하를 따라 아페리티보, 이어서 저녁. 첫날보다 여유 있게 밀라노의 밤을 즐깁니다.",
        "tip": "귀가는 지하철·택시. 소지품 주의.",
        "query": "나빌리오 그란데"
      }
    ]
  },
  {
    "date": "3.28",
    "dow": "일",
    "city": "밀라노 → 베로나",
    "title": "둘만의 로드트립 시작",
    "sub": "렌터카 수령, 로미오와 줄리엣의 도시",
    "stay": "베로나 · 구시가지 외곽 주차 가능한 숙소 / 1박",
    "move": "약 160km · 운전 2~2.5시간 + 연휴 정체",
    "note": "부활절 일요일 + 서머타임 시작(02시→03시). 렌터카 영업소의 휴일 수령 가능 여부·정확한 주소를 예약 시 확인하세요.",
    "spots": [
      {
        "time": "09:00–10:00",
        "name": "밀라노 중앙역 · 렌트 수령 권역",
        "lat": 45.4857,
        "lng": 9.2044,
        "kind": "차량",
        "desc": "예약한 영업소에서 차량 인수·외관 촬영·보험 범위 확인 후 베로나로 이동. 지도 핀은 중앙역 기준이며 실제 지점은 예약 후 교체해야 합니다.",
        "tip": "MXP 반납 편도 조건, 겨울 장비, 자동변속기, 국제운전면허증·한국 면허·운전자 명의 카드 확인.",
        "query": "밀라노 중앙역 · 렌트 수령 권역"
      },
      {
        "time": "12:30",
        "name": "Parking Cittadella · 베로나",
        "lat": 45.4353,
        "lng": 10.991,
        "kind": "주차",
        "desc": "주차 후 짐을 숙소에 맡기고 구시가지는 걸어서 이동.",
        "tip": "주차장 진입 경로·ZTL 표지 확인. 차량에 짐이 보이게 두지 않기.",
        "query": "Parking Cittadella Verona"
      },
      {
        "time": "14:00–15:00",
        "name": "베로나 아레나",
        "lat": 45.4389,
        "lng": 10.9944,
        "kind": "관람",
        "desc": "브라 광장에서 점심 후 원형극장 외관 감상. 내부는 해당일 개장 확인 시 선택.",
        "tip": "",
        "query": "베로나 아레나"
      },
      {
        "time": "15:30–17:00",
        "name": "에르베 광장",
        "lat": 45.443,
        "lng": 10.9978,
        "kind": "산책",
        "desc": "줄리엣의 집 주변 골목과 에르베 광장 산책. 줄이 길면 안뜰 입장에 집착하지 않기.",
        "tip": "",
        "query": "에르베 광장"
      },
      {
        "time": "18:00",
        "name": "피에트라 다리",
        "lat": 45.4478,
        "lng": 10.9996,
        "kind": "전망",
        "desc": "아디제 강변과 다리에서 사진을 찍고 구시가지에서 저녁.",
        "tip": "연휴 저녁 식사는 미리 예약.",
        "query": "피에트라 다리"
      }
    ],
    "parking": [
      {
        "name": "Cittadella · 베로나",
        "query": "Parcheggio Cittadella Verona",
        "desc": "기존 주차 후보 유지. 구시가지 관광 전 주차하고 도보 이동. 운영·차량 높이·숙박 주차 조건 확인.",
        "url": "https://www.amt3.it/"
      }
    ]
  },
  {
    "date": "3.29",
    "dow": "월",
    "city": "베로나 → 돌로미티",
    "title": "알프스의 작은 마을로",
    "sub": "오르티세이에 짐을 풀고 3박",
    "stay": "오르티세이 · 주차·스파가 있는 숙소 / 3박 중 1박",
    "move": "약 190km · 운전 2.5~3.5시간 + 휴식",
    "note": "부활절 월요일. A22–키우사 경유로 접근하고 산악 고개를 넘는 지름길은 피합니다. 늦어도 밝을 때 체크인하세요.",
    "spots": [
      {
        "time": "09:30",
        "name": "베로나 출발 · Cittadella",
        "lat": 45.4353,
        "lng": 10.991,
        "kind": "차량",
        "desc": "아침 식사 후 출발. 고속도로 휴게소에서 커피·화장실 휴식 1회.",
        "tip": "",
        "query": "베로나 출발 · Cittadella"
      },
      {
        "time": "13:00–14:00",
        "name": "오르티세이 중심가",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "마을",
        "desc": "도착 후 티롤식 점심. 카네델리와 따뜻한 수프로 시작합니다.",
        "tip": "",
        "query": "오르티세이 중심가"
      },
      {
        "time": "15:00–17:00",
        "name": "산트울리코 교회 일대",
        "lat": 46.5758,
        "lng": 11.6723,
        "kind": "휴식",
        "desc": "호텔 체크인 후 보행자 거리만 가볍게 산책. 이후 스파와 저녁 식사.",
        "tip": "스파 운영·커플 이용 조건은 숙소에 확인. 이날 높은 산 일정은 잡지 않기.",
        "query": "산트울리코 교회 일대"
      }
    ],
    "parking": [
      {
        "name": "오르티세이 숙소 주차 우선",
        "query": "Parcheggio Central Ortisei",
        "desc": "체크인 전에는 Parcheggio Central을 검색해 운영·진입로 확인. 3박은 숙소 주차 가능 여부를 먼저 확정.",
        "url": ""
      }
    ]
  },
  {
    "date": "3.30",
    "dow": "화",
    "city": "돌로미티",
    "title": "세체다, 하얀 능선을 만나는 날",
    "sub": "맑은 날에 우선 배치",
    "stay": "오르티세이 / 3박 중 2박",
    "move": "도보 + 케이블카 · 운전 최소화",
    "note": "여름 초원 하이킹 일정이 아닙니다. 리프트 운행·보행자 탑승·정상 접근 가능 여부를 아침에 확인. 3/31과 날씨에 따라 교환하세요. 별도 날씨 예비일은 줄였습니다. 악천후에는 볼차노 실내 관람으로 대체하세요.",
    "spots": [
      {
        "time": "09:00",
        "name": "세체다 케이블카 하부역",
        "lat": 46.5779,
        "lng": 11.6748,
        "kind": "조건부",
        "desc": "보행자 왕복권과 운행 상황을 확인하고 탑승. 강풍·안개면 볼차노 대안으로 전환.",
        "tip": "미끄럼 방지 신발·방풍 외투 준비. 스키 슬로프에 들어가지 않기.",
        "query": "세체다 케이블카 하부역"
      },
      {
        "time": "10:00–12:30",
        "name": "세체다 전망 권역",
        "lat": 46.6003,
        "lng": 11.7244,
        "kind": "전망",
        "desc": "운영자가 허용하는 구역에서 설산 전망과 사진. 열린 산장에서 느긋하게 점심.",
        "tip": "핀은 전망 권역의 대략 위치. 능선 보행은 적설·통제 상황에 따라 생략.",
        "query": "세체다 전망 권역"
      },
      {
        "time": "15:00",
        "name": "오르티세이 보행자 거리",
        "lat": 46.5744,
        "lng": 11.6707,
        "kind": "휴식",
        "desc": "하산 후 카페와 숙소 스파. 저녁은 호텔 또는 마을 레스토랑.",
        "tip": "",
        "query": "오르티세이 보행자 거리"
      }
    ]
  },
  {
    "date": "3.31",
    "dow": "수",
    "city": "돌로미티",
    "title": "알페 디 시우시의 느린 하루",
    "sub": "설원 전망과 산장 점심",
    "stay": "오르티세이 / 3박 중 3박",
    "move": "도보 + 케이블카",
    "note": "고원까지 차량 진입하는 일정 대신 오르티세이 출발 리프트 이용. 해당 시설의 겨울 운영과 보행자 이용을 별도로 확인하세요.",
    "spots": [
      {
        "time": "09:30",
        "name": "오르티세이–알페 디 시우시 케이블카",
        "lat": 46.5726,
        "lng": 11.6712,
        "kind": "조건부",
        "desc": "호텔에서 하부역으로 이동. 날씨와 보행자 탑승 확인 후 왕복 이용.",
        "tip": "",
        "query": "오르티세이–알페 디 시우시 케이블카"
      },
      {
        "time": "10:00–13:30",
        "name": "몬트 세우크 전망 권역",
        "lat": 46.5575,
        "lng": 11.6655,
        "kind": "전망",
        "desc": "사솔룽고 방향 설산을 감상하고 영업 중인 산장에서 점심. 개방된 겨울 산책로만 짧게 이용.",
        "tip": "고원 전체 트레킹·초원 피크닉은 계획하지 않습니다.",
        "query": "몬트 세우크 전망 권역"
      },
      {
        "time": "15:00",
        "name": "오르티세이 마을",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "휴식",
        "desc": "일찍 하산해서 낮잠·카페. 저녁은 티롤 음식과 디저트.",
        "tip": "",
        "query": "오르티세이 마을"
      }
    ]
  },
  {
    "date": "4.1",
    "dow": "목",
    "city": "돌로미티 → 베네치아",
    "title": "설산에서 물의 도시로",
    "sub": "차는 섬 입구에, 여행은 배 위에서",
    "stay": "베네치아 본섬 · 산타루치아역/산폴로 인근 / 1박",
    "move": "약 300km · 운전 3.5~4.5시간 + 수상 이동",
    "note": "A22–A4 경유를 기본으로 두고 산악 고개 횡단은 제외. 본섬 숙박용 작은 가방을 준비하되 귀중품은 모두 소지합니다.",
    "spots": [
      {
        "time": "09:00",
        "name": "오르티세이 출발",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "차량",
        "desc": "조식 후 출발. 중간 휴식 포함 오후 초반 주차장 도착 목표.",
        "tip": "",
        "query": "오르티세이 출발"
      },
      {
        "time": "13:30",
        "name": "트론케토 주차장",
        "lat": 45.4423,
        "lng": 12.3056,
        "kind": "주차",
        "desc": "차를 주차하고 바포레토 또는 피플무버 연계로 숙소 이동.",
        "tip": "24시간 이상 주차·수상버스 표 확인. 본섬 호텔 주소를 차량 내비 목적지로 넣지 않기.",
        "query": "트론케토 주차장"
      },
      {
        "time": "16:00",
        "name": "리알토 다리",
        "lat": 45.438,
        "lng": 12.3359,
        "kind": "산책",
        "desc": "체크인 후 대운하와 골목 산책. 바카로에서 치케티로 간단히 요기.",
        "tip": "",
        "query": "리알토 다리"
      },
      {
        "time": "17:30–19:30",
        "name": "산마르코 광장",
        "lat": 45.4342,
        "lng": 12.3388,
        "kind": "전망",
        "desc": "저녁빛의 광장과 해안 산책. 곤돌라는 날씨·대기·현장 요금을 보고 선택.",
        "tip": "2027년 입도료 시행일과 숙박객 면제 절차는 공식 공지 재확인.",
        "query": "산마르코 광장"
      }
    ],
    "parking": [
      {
        "name": "Tronchetto Parking · 베네치아",
        "query": "Tronchetto Parking Venezia",
        "desc": "공식 안내 24시간 운영, ZTL 밖. 바포레토·People Mover 연계. 4/1 입차~4/2 출차 기준 요금 확인.",
        "url": "https://www.tronchettoparking.it/it/"
      }
    ]
  },
  {
    "date": "4.2",
    "dow": "금",
    "city": "베네치아 → 시에나",
    "title": "운하에서 토스카나로",
    "sub": "볼로냐 숙박 없이 시에나로 바로",
    "stay": "시에나 · 성벽 밖 주차 가능한 숙소 / 2박 중 1박 · 토스카나 1/4박",
    "move": "운전 약 4~5시간 + 휴식·출차 1~1.5시간 (계획용 추정)",
    "note": "베네치아 오전 관람 후 시에나로 바로 이동합니다. 볼로냐는 고속도로 경유만 하고 방문·숙박하지 않습니다. 이날 시에나 입장 예약은 잡지 않고 대성당은 다음 날 관람.",
    "parking": [
      {
        "name": "Santa Caterina · 기본 주차장",
        "query": "Parcheggio Santa Caterina Siena",
        "desc": "성벽 밖에 주차한 뒤 에스컬레이터로 구시가지 접근. 숙소 전용 주차가 있으면 우선 이용.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "name": "Il Campo · 대안",
        "query": "Parcheggio Il Campo Siena",
        "desc": "캄포 광장 방문 대안. 내비에 광장이 아니라 주차장명을 입력하고 ZTL 표지와 공식 진입 안내를 확인.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      }
    ],
    "spots": [
      {
        "time": "08:00–09:00",
        "name": "산마르코 광장",
        "lat": 45.4342,
        "lng": 12.3388,
        "kind": "산책",
        "desc": "아침 광장과 조식. 큰 짐은 체크아웃 동선에 맞춰 정리.",
        "tip": "",
        "query": "산마르코 광장",
        "url": ""
      },
      {
        "time": "09:00–10:00",
        "name": "산마르코 대성당",
        "lat": 45.4346,
        "lng": 12.3397,
        "kind": "예약",
        "desc": "공식 예약 가능 시간에 맞춰 관람. 오전 예약이 불가능하면 광장 산책으로 대체.",
        "tip": "예약 미확정. 10시 무렵 주차장으로 이동 시작.",
        "query": "산마르코 대성당",
        "url": ""
      },
      {
        "time": "11:00–11:30",
        "name": "Tronchetto Parking · 차량 회수",
        "lat": 45.4423,
        "lng": 12.3056,
        "kind": "주차",
        "desc": "차량 회수 후 시에나로 출발. 고속도로에서 점심·휴식 1회.",
        "tip": "본섬에서 주차장 이동 시간을 확보.",
        "query": "Tronchetto Parking Venezia",
        "url": "https://www.tronchettoparking.it/it/"
      },
      {
        "time": "16:30–17:30",
        "name": "Santa Caterina · 시에나 주차",
        "lat": 43.3162,
        "lng": 11.326,
        "kind": "주차",
        "desc": "숙소 주차 또는 Santa Caterina 이용 후 체크인. 도착 시간은 교통 상황에 따라 조정.",
        "tip": "지도 핀은 대표 위치. 아래 주차장명 길찾기와 현장 진입 표지를 우선.",
        "query": "Parcheggio Santa Caterina Siena",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "time": "18:00–20:00",
        "name": "캄포 광장",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "짧은 골목 산책과 피치 파스타 저녁. 장거리 이동 후 충분히 휴식.",
        "tip": "",
        "query": "캄포 광장",
        "url": ""
      }
    ]
  },
  {
    "date": "4.3",
    "dow": "토",
    "city": "시에나",
    "title": "시에나에 온전히 머무는 하루",
    "sub": "대성당·캄포 광장·느린 골목 산책",
    "stay": "시에나 / 2박 중 2박 · 토스카나 2/4박",
    "move": "도보 중심 · 차량은 숙소 또는 같은 주차장에",
    "note": "추가한 하루는 시에나를 여유 있게 보는 날로 사용합니다. 숙박 중 차를 옮기지 않고 다음 날 발도르차로 출발. 주차요금은 실제 체류시간으로 확인.",
    "parking": [
      {
        "name": "Santa Caterina · 기본 주차장",
        "query": "Parcheggio Santa Caterina Siena",
        "desc": "성벽 밖에 주차한 뒤 에스컬레이터로 구시가지 접근. 숙소 전용 주차가 있으면 우선 이용.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "name": "Il Campo · 대안",
        "query": "Parcheggio Il Campo Siena",
        "desc": "캄포 광장 방문 대안. 내비에 광장이 아니라 주차장명을 입력하고 ZTL 표지와 공식 진입 안내를 확인.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      }
    ],
    "spots": [
      {
        "time": "09:30–11:30",
        "name": "시에나 대성당",
        "lat": 43.3177,
        "lng": 11.3289,
        "kind": "예약",
        "desc": "대성당과 통합권 포함 시설을 취향에 맞춰 관람. 예약 가능 시간 확인.",
        "tip": "",
        "query": "시에나 대성당",
        "url": ""
      },
      {
        "time": "12:00–14:00",
        "name": "캄포 광장",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "점심 후 광장에 앉아 쉬기. 만자 탑은 체력과 입장 상황에 따라 선택.",
        "tip": "",
        "query": "캄포 광장",
        "url": ""
      },
      {
        "time": "14:30–16:00",
        "name": "산 도메니코 성당",
        "lat": 43.3204,
        "lng": 11.3272,
        "kind": "산책",
        "desc": "구시가지 골목과 성당 주변 전망 산책. 피곤하면 카페·숙소 휴식.",
        "tip": "",
        "query": "산 도메니코 성당",
        "url": ""
      },
      {
        "time": "18:30",
        "name": "시에나 구시가지",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "도보로 저녁 식사. 다음 날 농가 이동을 위해 짐 정리.",
        "tip": "",
        "query": "시에나 구시가지",
        "url": ""
      }
    ]
  },
  {
    "date": "4.4",
    "dow": "일",
    "city": "시에나 → 발도르차 → 농가",
    "title": "와인 마을을 지나 농가로",
    "sub": "몬탈치노·바뇨 비뇨니·피엔차",
    "stay": "Fattoria del Colle · Trequanda / 추천 숙소 2박 중 1박 · 토스카나 3/4박 · 예약 미확정",
    "move": "시에나 → 몬탈치노 → 바뇨 비뇨니 → 피엔차 → 트레콴다 · 총 운전 약 2.5~3시간",
    "note": "서쪽 몬탈치노에서 동쪽 피엔차를 거쳐 농가로 이동합니다. 이날은 마을 관광, 와인 시음은 다음 날 숙소 현장 투어에 배치. 모든 시간은 주차·교통에 따라 조정.",
    "parking": [
      {
        "name": "몬탈치노 · Fortezza 주변 주차 후보",
        "query": "Parcheggio Fortezza Montalcino",
        "desc": "성곽 바깥 주차 후보. 정확한 입구·공사·요금은 현지 표지에서 확인. 지도 검색 결과에서 주차장 선택.",
        "url": ""
      },
      {
        "name": "바뇨 비뇨니 · 마을 입구 공영주차",
        "query": "Parcheggio Bagno Vignoni",
        "desc": "중앙 수조로 진입하지 말고 마을 입구 주차 표지를 따르기. 현지 운영 확인.",
        "url": ""
      },
      {
        "name": "피엔차 · 성벽 밖 공영주차",
        "query": "Parcheggio Pienza",
        "desc": "역사 중심부를 목적지로 운전하지 말고 성벽 밖 주차장을 선택. 일요일 혼잡 시 여유 시간 확보.",
        "url": ""
      }
    ],
    "spots": [
      {
        "time": "09:00",
        "name": "Santa Caterina · 시에나 출발",
        "lat": 43.3162,
        "lng": 11.326,
        "kind": "차량",
        "desc": "체크아웃 후 차량 회수. 짐은 외부에서 보이지 않게 정리.",
        "tip": "",
        "query": "Parcheggio Santa Caterina Siena",
        "url": ""
      },
      {
        "time": "10:15–11:45",
        "name": "몬탈치노 성곽·구시가지",
        "lat": 43.0568,
        "lng": 11.4892,
        "kind": "마을",
        "desc": "브루넬로의 마을에서 성곽과 전망 산책. 운전자는 시음하지 않고 병 구매만.",
        "tip": "성곽 바깥 주차 후 도보.",
        "query": "Fortezza Montalcino",
        "url": "https://www.visittuscany.com/en/towns-and-villages/montalcino/"
      },
      {
        "time": "12:15–13:15",
        "name": "바뇨 비뇨니",
        "lat": 43.0287,
        "lng": 11.6183,
        "kind": "마을",
        "desc": "온천 광장 산책과 가벼운 점심. 중앙 수조는 목욕 장소가 아닙니다.",
        "tip": "",
        "query": "바뇨 비뇨니",
        "url": ""
      },
      {
        "time": "14:00–15:30",
        "name": "피엔차",
        "lat": 43.0775,
        "lng": 11.6797,
        "kind": "마을",
        "desc": "전망 산책로와 골목, 페코리노 치즈 가게. 농가 체크인을 위해 오후 일찍 출발.",
        "tip": "",
        "query": "피엔차",
        "url": ""
      },
      {
        "time": "16:30",
        "name": "Fattoria del Colle · 체크인",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "숙박",
        "desc": "추천 농가에서 2박. 트레콴다 권역 핀은 대략 위치이며 길찾기는 숙소명으로 연결됩니다.",
        "tip": "예약 시 전용 주차·진입로·체크인 마감·4/4와 4/5 저녁 운영 확인.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      },
      {
        "time": "19:00",
        "name": "Fattoria del Colle · 농가 저녁",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "식사",
        "desc": "식당 운영과 예약 확정 시 농가에서 저녁. 운영하지 않으면 시음 전 이동 가능한 식당 또는 숙소 식사 대안을 준비.",
        "tip": "숙소·식사·투어 모두 예약 미확정.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      }
    ]
  },
  {
    "date": "4.5",
    "dow": "월",
    "city": "토스카나 · 농가",
    "title": "포도밭과 와인, 느린 하루",
    "sub": "농가 산책·저장고 투어·숙소 저녁",
    "stay": "Fattoria del Colle · Trequanda / 추천 숙소 2박 중 2박 · 토스카나 4/4박 · 예약 미확정",
    "move": "숙소 중심 · 선택 시 트레콴다 마을 짧은 왕복",
    "note": "둘 다 와인을 즐길 수 있도록 오후 투어부터 차를 사용하지 않습니다. 숙박과 투어는 별도 확정하며, 몬탈치노의 Casato Prime Donne가 아닌 Fattoria del Colle 현장 프로그램을 선택하세요.",
    "spots": [
      {
        "time": "09:00–11:00",
        "name": "Fattoria del Colle · 농가 산책",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "휴식",
        "desc": "조식 후 농가 부지의 개방된 산책 구역에서 휴식. 4월에는 포도 수확 체험을 전제로 하지 않습니다.",
        "tip": "정확한 위치·주차는 숙소 안내 기준.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      },
      {
        "time": "11:30–13:30",
        "name": "트레콴다 · 선택 산책",
        "lat": 43.1884,
        "lng": 11.6688,
        "kind": "마을",
        "desc": "컨디션이 좋으면 가까운 마을에서 점심. 차량 이동을 마치고 농가로 돌아온 뒤 시음.",
        "tip": "구시가지 바깥 주차 표지 이용. 쉬고 싶으면 농가에 머물기.",
        "query": "Trequanda",
        "url": ""
      },
      {
        "time": "15:30–16:30",
        "name": "Fattoria del Colle · 와인 투어",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "예약",
        "desc": "빌라·정원·역사적 저장고와 와인 3종 시음. 공식 현행 안내 약 1시간, 1인 €29.50; 2027년 가격·월요일 운영·영어 진행은 예약 시 재확인.",
        "tip": "투어 후 운전 일정 없음. 숙소 안에서 진행되는 프로그램으로 예약.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/dettaglio/sale-della-villa-del-cinquecento-cappella-giardini-antichi-cantina-storica-con-degustazione-itinerante/"
      },
      {
        "time": "17:00–저녁",
        "name": "Fattoria del Colle · 휴식과 저녁",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "휴식",
        "desc": "농가에서 느긋한 저녁. 다음 날 피렌체로 이동할 준비.",
        "tip": "당일 식당 운영과 저녁 예약 필수 확인.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      }
    ]
  },
  {
    "date": "4.6",
    "dow": "화",
    "city": "토스카나 → 피렌체",
    "title": "농가에서 르네상스의 도시로",
    "sub": "오전 이동 후 기존 피렌체 일정",
    "stay": "피렌체 · ZTL 밖 주차 가능한 숙소 / 2박 중 1박",
    "move": "트레콴다 → A1 → Villa Costanza · 운전 약 1.5~2시간 + 트램·주차",
    "note": "농가에서 조식 후 09시 무렵 출발. A1 경유로 Villa Costanza 주차장에 진입하고 트램 T1으로 도심 이동. 피렌체 2박과 우피치 관람 날짜는 유지.",
    "spots": [
      {
        "time": "11:00",
        "name": "Villa Costanza 주차장",
        "lat": 43.7541,
        "lng": 11.1734,
        "kind": "주차",
        "desc": "A1에서 전용 출구로 진입해 주차. 트램 T1으로 피렌체 도심 이동 후 짐 보관.",
        "tip": "2027년 요금·차량 조건 재확인. 호텔 전용 주차를 선택하면 실제 출발·회수 장소도 함께 변경.",
        "query": "Parcheggio Villa Costanza Scandicci",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      },
      {
        "time": "13:00–15:00",
        "name": "피렌체 두오모",
        "lat": 43.7731,
        "lng": 11.256,
        "kind": "관람",
        "desc": "점심 후 대성당 광장과 주변 골목 산책. 돔 등반은 원하면 예약 시간에 맞춰 추가.",
        "tip": "돔 등반은 체력·계단을 고려. 이 기본안은 외관 중심.",
        "query": "피렌체 두오모"
      },
      {
        "time": "15:30",
        "name": "시뇨리아 광장",
        "lat": 43.7696,
        "lng": 11.2558,
        "kind": "산책",
        "desc": "야외 조각을 보고 젤라토 휴식.",
        "tip": "",
        "query": "시뇨리아 광장"
      },
      {
        "time": "17:00–19:00",
        "name": "베키오 다리",
        "lat": 43.768,
        "lng": 11.2531,
        "kind": "전망",
        "desc": "아르노 강변을 걷고 올트라르노에서 저녁.",
        "tip": "",
        "query": "베키오 다리"
      }
    ],
    "parking": [
      {
        "name": "Villa Costanza · 피렌체 2박",
        "query": "Parcheggio Villa Costanza Scandicci",
        "desc": "A1 고속도로에서만 차량 진입. 트램 T1으로 SMN역까지 공식 안내 약 22분. 최대 30일 연속 주차 가능, 트램 별도 결제. 2027년 요금 재확인.",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      }
    ]
  },
  {
    "date": "4.7",
    "dow": "수",
    "city": "피렌체",
    "title": "예술 한 번, 노을 한 번",
    "sub": "우피치와 도시를 내려다보는 저녁",
    "stay": "피렌체 / 2박 중 2박",
    "move": "도보 중심 · 전망대는 버스·택시 가능",
    "note": "대형 미술관은 하루 하나만. 우피치를 수요일 오전에 배치했습니다. 아래 시간은 예약 목표이며 확정 입장권이 아닙니다.",
    "spots": [
      {
        "time": "09:00–12:00",
        "name": "우피치 미술관",
        "lat": 43.7678,
        "lng": 11.2553,
        "kind": "예약",
        "desc": "보티첼리와 르네상스 작품 중심으로 약 3시간 관람. 공식 시간 지정 예약 추천.",
        "tip": "",
        "query": "우피치 미술관"
      },
      {
        "time": "12:30–15:00",
        "name": "산토 스피리토 광장",
        "lat": 43.7665,
        "lng": 11.2478,
        "kind": "식사",
        "desc": "올트라르노에서 점심 후 공방 골목. 중간에 호텔에서 쉬어도 좋습니다.",
        "tip": "",
        "query": "산토 스피리토 광장"
      },
      {
        "time": "17:30–19:30",
        "name": "미켈란젤로 광장",
        "lat": 43.7629,
        "lng": 11.2651,
        "kind": "전망",
        "desc": "해 지기 전 도착해 피렌체 전경과 노을 사진. 실제 일몰·날씨에 맞춰 시간을 조정.",
        "tip": "오르막은 버스·택시로 이동 가능. 차는 숙소에 두기.",
        "query": "미켈란젤로 광장"
      },
      {
        "time": "20:00",
        "name": "산토 스피리토 일대",
        "lat": 43.7665,
        "lng": 11.2478,
        "kind": "식사",
        "desc": "여행 후반의 기념 저녁. 비스테카는 2인 양·가격을 먼저 확인 후 주문.",
        "tip": "",
        "query": "산토 스피리토 일대"
      }
    ],
    "parking": [
      {
        "name": "Villa Costanza · 피렌체 2박",
        "query": "Parcheggio Villa Costanza Scandicci",
        "desc": "A1 고속도로에서만 차량 진입. 트램 T1으로 SMN역까지 공식 안내 약 22분. 최대 30일 연속 주차 가능, 트램 별도 결제. 2027년 요금 재확인.",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      }
    ]
  },
  {
    "date": "4.8",
    "dow": "목",
    "city": "피렌체 → 파르마",
    "title": "미식 도시에서 마지막 밤",
    "sub": "산을 넘어 에밀리아로",
    "stay": "파르마 · 구시가지 외곽 주차 가능한 숙소 / 1박",
    "move": "약 190km · 운전 2.5~3시간",
    "note": "아침 치즈 공장 투어는 오늘 이동과 맞추기 어렵습니다. 이번 기본안에서는 파르마 도심과 여유로운 마지막 저녁을 선택합니다.",
    "spots": [
      {
        "time": "09:30",
        "name": "Villa Costanza · 차량 회수",
        "lat": 43.7541,
        "lng": 11.1734,
        "kind": "차량",
        "desc": "실제 주차 장소에서 차를 찾아 파르마로 출발. 고속도로 휴식 1회.",
        "tip": "",
        "query": "Villa Costanza · 차량 회수"
      },
      {
        "time": "13:00",
        "name": "Parcheggio Toschi · 파르마",
        "lat": 44.8076,
        "lng": 10.3271,
        "kind": "주차",
        "desc": "주차·체크인 후 점심. 진입 경로는 주차장 공식 안내 확인.",
        "tip": "",
        "query": "Parcheggio Toschi Parma"
      },
      {
        "time": "14:30–16:00",
        "name": "파르마 대성당",
        "lat": 44.8035,
        "lng": 10.3302,
        "kind": "관람",
        "desc": "대성당과 세례당 광장 산책. 내부는 개관·입장권 확인 후 관람.",
        "tip": "",
        "query": "파르마 대성당"
      },
      {
        "time": "16:30",
        "name": "팔라초 델라 필로타",
        "lat": 44.8065,
        "lng": 10.3263,
        "kind": "산책",
        "desc": "광장 산책 또는 운영 시 테아트로 파르네세 관람 중 선택.",
        "tip": "",
        "query": "팔라초 델라 필로타"
      },
      {
        "time": "19:00",
        "name": "가리발디 광장 · 파르마",
        "lat": 44.8012,
        "lng": 10.328,
        "kind": "식사",
        "desc": "프로슈토 디 파르마, 파르미자노 레자노, 토르텔리로 마지막 만찬. 숙소까지 도보.",
        "tip": "",
        "query": "가리발디 광장 Parma"
      }
    ],
    "parking": [
      {
        "name": "Toschi · 파르마",
        "query": "Parcheggio Toschi Parma",
        "desc": "기존 주차 후보 유지. 도심 관광·1박 주차는 숙소 전용 주차와 비교하고 진입로·운영을 출발 전 확인.",
        "url": ""
      }
    ]
  },
  {
    "date": "4.9",
    "dow": "금",
    "city": "파르마 → 밀라노 공항",
    "title": "Arrivederci, Italia",
    "sub": "마지막 날은 비행기에 여유 있게",
    "stay": "기내 · 한국 도착 날짜는 항공권 확인",
    "move": "MXP까지 약 190~210km · 운전 2.5~3시간 + 정체 여유",
    "note": "KE928 · MXP 22:00 출발은 이탈리아 현지 시간. 반납은 MXP를 우선 추천하되 계약에서 밀라노 시내 수령→MXP 반납을 확정하세요.",
    "spots": [
      {
        "time": "09:30–11:00",
        "name": "파르마 가리발디 광장",
        "lat": 44.8012,
        "lng": 10.328,
        "kind": "식사",
        "desc": "늦은 아침과 식품 쇼핑. 점심까지 파르마에서 해결하고 짐 정리.",
        "tip": "",
        "query": "파르마 가리발디 광장"
      },
      {
        "time": "13:00",
        "name": "Parcheggio Toschi · 출발",
        "lat": 44.8076,
        "lng": 10.3271,
        "kind": "차량",
        "desc": "공항으로 바로 이동. 밀라노 도심 재진입이나 아웃렛 쇼핑은 기본안에서 제외.",
        "tip": "금요일 정체 대비. 주유는 반납점의 지정 조건에 맞추고 영수증 보관.",
        "query": "Parcheggio Toschi · 출발"
      },
      {
        "time": "17:30–18:00",
        "name": "MXP · 렌터카 반납 권역",
        "lat": 45.6301,
        "lng": 8.7231,
        "kind": "차량",
        "desc": "예약 영업소·터미널의 정확한 반납 구역으로 이동. 차량 사진·반납 확인서 확보.",
        "tip": "핀은 공항 대표 위치. 실제 반납점은 렌트 계약서 기준. 시내 반납이면 공항 이동 1~1.5시간 이상 별도 추가.",
        "query": "MXP · 렌터카 반납 권역"
      },
      {
        "time": "19:00 / 22:00",
        "name": "밀라노 말펜사 공항 · 출국",
        "lat": 45.6301,
        "lng": 8.7231,
        "kind": "공항",
        "desc": "19시까지 출발 터미널 도착 목표. 체크인·수하물·보안 검색 후 22시 KE928 탑승.",
        "tip": "항공사 안내가 더 이른 도착을 요구하면 그 기준을 우선.",
        "query": "밀라노 말펜사 공항 · 출국"
      }
    ]
  }
];
