const days = [
  {
    "date": "3.26",
    "dow": "금",
    "city": "밀라노",
    "title": "Ciao, Italia",
    "sub": "도착하는 날은 가볍게",
    "stay": "밀라노 · 중앙역 또는 브레라 인근 / 2박 중 1박",
    "move": "공항 → 시내 약 1~1.5시간",
    "note": "KE927 · ICN 12:15 출발은 한국 시간. MXP 도착 시각은 항공권 확인 후 아래 일정을 조정하세요. 이날은 입장 예약을 잡지 않습니다.",
    "spots": [
      {
        "time": "도착 후",
        "name": "밀라노 말펜사 공항",
        "lat": 45.6301,
        "lng": 8.7231,
        "kind": "공항",
        "desc": "입국·수하물 수령에 1~2시간 여유를 두고 시내로 이동. Malpensa Express 또는 택시를 이용합니다.",
        "tip": "터미널과 운행 시간은 항공권·공식 교통편에서 확인.",
        "query": "밀라노 말펜사 공항"
      },
      {
        "time": "체크인 후",
        "name": "브레라 지구",
        "lat": 45.4719,
        "lng": 9.1876,
        "kind": "산책",
        "desc": "호텔에 짐을 놓고 골목을 천천히 걷기. 첫 저녁은 리소토 알라 밀라네제와 가벼운 식사.",
        "tip": "피곤하면 산책을 생략하고 호텔 근처에서 식사.",
        "query": "브레라 지구"
      }
    ]
  },
  {
    "date": "3.27",
    "dow": "토",
    "city": "밀라노",
    "title": "지붕 위에서 시작하는 여행",
    "sub": "두오모와 브레라, 운하의 저녁",
    "stay": "밀라노 / 2박 중 2박",
    "move": "도보 + 지하철 · 렌터카 없이",
    "note": "두오모는 오전 예약을 추천. 최후의 만찬은 취향에 따라 브레라 산책 대신 넣되, 예매 성공 후 확정하세요.",
    "spots": [
      {
        "time": "09:00–11:00",
        "name": "밀라노 두오모",
        "lat": 45.4642,
        "lng": 9.1916,
        "kind": "예약",
        "desc": "대성당과 루프톱을 함께 관람. 첨탑 사이에서 둘만의 사진을 남기세요.",
        "tip": "공식 사이트에서 입장 시간 예약. 예배·행사에 따라 관람 변경 가능.",
        "query": "밀라노 두오모"
      },
      {
        "time": "11:00–12:00",
        "name": "비토리오 에마누엘레 2세 갤러리아",
        "lat": 45.4658,
        "lng": 9.19,
        "kind": "산책",
        "desc": "아케이드 구경과 커피. 점심은 주변에서 코톨레타 또는 파스타.",
        "tip": "",
        "query": "비토리오 에마누엘레 2세 갤러리아"
      },
      {
        "time": "14:00–16:00",
        "name": "브레라 미술관 일대",
        "lat": 45.4719,
        "lng": 9.1881,
        "kind": "산책",
        "desc": "미술관 관람 또는 디자인 상점과 골목 산책 중 컨디션에 맞춰 선택.",
        "tip": "",
        "query": "브레라 미술관 일대"
      },
      {
        "time": "18:00–20:00",
        "name": "나빌리오 그란데",
        "lat": 45.4505,
        "lng": 9.1706,
        "kind": "식사",
        "desc": "운하를 따라 아페리티보, 이어서 저녁. 첫날보다 여유 있게 밀라노의 밤을 즐깁니다.",
        "tip": "귀가는 지하철·택시. 소지품 주의.",
        "query": "나빌리오 그란데"
      }
    ]
  },
  {
    "date": "3.28",
    "dow": "일",
    "city": "밀라노 → 베로나",
    "title": "둘만의 로드트립 시작",
    "sub": "렌터카 수령, 로미오와 줄리엣의 도시",
    "stay": "베로나 · 구시가지 외곽 주차 가능한 숙소 / 1박",
    "move": "약 160km · 운전 2~2.5시간 + 연휴 정체",
    "note": "Hertz 예약서 기준: 2027/3/28(일) 10:00 밀라노 중앙역 영업소 수령. 수령 주소 Via Cappellini 10, Milano 20124. 예약서의 일반 일요일 영업시간 08:00–18:00. 부활절 특별 운영·차량 준비는 출발 전 확인. 이날 서머타임 시작으로 현지 시계가 1시간 빨라집니다.",
    "spots": [
      {
        "time": "09:45 도착 / 10:00 예약",
        "name": "Hertz · 밀라노 중앙역 차량 수령",
        "lat": 45.4804,
        "lng": 9.2036,
        "kind": "예약 완료",
        "desc": "Via Cappellini 10, Milano 20124에서 수령. 예약 차급은 자동변속 Compact 4–5 Door, 예시 차량 Jeep Avenger (Y0). 인수·외관 촬영 후 10:45~11:00 출발 목표.",
        "tip": "여권·한국 운전면허증·국제운전면허증·운전자 명의 신용카드 지참. 연료 종류·반납 연료 조건·겨울 장비의 실제 제공 품목은 인수 시 확인. 지도 핀은 주소 주변의 대표 위치.",
        "query": "Hertz Via Cappellini 10 Milano",
        "url": ""
      },
      {
        "time": "13:00–13:30",
        "name": "Parking Cittadella · 베로나",
        "lat": 45.4353,
        "lng": 10.991,
        "kind": "주차",
        "desc": "주차 후 짐을 숙소에 맡기고 구시가지는 걸어서 이동.",
        "tip": "주차장 진입 경로·ZTL 표지 확인. 차량에 짐이 보이게 두지 않기.",
        "query": "Parking Cittadella Verona"
      },
      {
        "time": "14:30–15:30",
        "name": "베로나 아레나",
        "lat": 45.4389,
        "lng": 10.9944,
        "kind": "관람",
        "desc": "브라 광장에서 점심 후 원형극장 외관 감상. 내부는 해당일 개장 확인 시 선택.",
        "tip": "",
        "query": "베로나 아레나"
      },
      {
        "time": "15:30–17:00",
        "name": "에르베 광장",
        "lat": 45.443,
        "lng": 10.9978,
        "kind": "산책",
        "desc": "줄리엣의 집 주변 골목과 에르베 광장 산책. 줄이 길면 안뜰 입장에 집착하지 않기.",
        "tip": "",
        "query": "에르베 광장"
      },
      {
        "time": "18:00",
        "name": "피에트라 다리",
        "lat": 45.4478,
        "lng": 10.9996,
        "kind": "전망",
        "desc": "아디제 강변과 다리에서 사진을 찍고 구시가지에서 저녁.",
        "tip": "연휴 저녁 식사는 미리 예약.",
        "query": "피에트라 다리"
      }
    ],
    "parking": [
      {
        "name": "Cittadella · 베로나",
        "query": "Parcheggio Cittadella Verona",
        "desc": "기존 주차 후보 유지. 구시가지 관광 전 주차하고 도보 이동. 운영·차량 높이·숙박 주차 조건 확인.",
        "url": "https://www.amt3.it/"
      }
    ]
  },
  {
    "date": "3.29",
    "dow": "월",
    "city": "베로나 → 돌로미티",
    "title": "베로나에서 오르티세이로, 산은 다음 날",
    "sub": "이동·체크인·마을 산책으로 컨디션 확보",
    "stay": "오르티세이 중심부 · 주차·스파 가능한 숙소 / 3박 중 1박",
    "move": "약 190km · 운전 2.5~3.5시간 + 휴식",
    "note": "부활절 월요일. A22–키우사 경유로 접근하고 산악 고개 지름길은 피합니다. 3박 동안 숙소를 옮기지 않고 오르티세이를 베이스로 사용합니다. 이날은 리프트를 타지 않고 체크인·동선 확인·휴식에 집중합니다.",
    "spots": [
      {
        "time": "09:30",
        "name": "베로나 출발 · Cittadella",
        "lat": 45.4353,
        "lng": 10.991,
        "kind": "차량",
        "desc": "아침 식사 후 출발. A22에서 커피·화장실 휴식 1회를 잡고 무리해서 직행하지 않습니다.",
        "tip": "부활절 연휴 교통량과 휴게소 혼잡을 감안해 도착 시간을 30~60분 여유 있게 봅니다.",
        "query": "베로나 출발 · Cittadella"
      },
      {
        "time": "13:00–14:00",
        "name": "오르티세이 중심가",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "도착",
        "desc": "숙소 주차 후 체크인 또는 짐 보관. 늦은 점심은 마을 안에서 해결하고 차량은 가능하면 그대로 둡니다.",
        "tip": "숙소는 Seceda 하부역과 마을 중심을 도보로 오갈 수 있는 위치를 우선합니다.",
        "query": "Ortisei center"
      },
      {
        "time": "15:30–16:30",
        "name": "세체다 케이블카 하부역 사전 확인",
        "lat": 46.5779,
        "lng": 11.6748,
        "kind": "동선 확인",
        "desc": "다음 날 탈 Ortisei–Furnes 하부역까지 걸어가 입구와 매표 위치를 확인합니다. 탑승은 다음 날로 미룹니다.",
        "tip": "2026/27 공식 운행기간은 2026-12-04~2027-04-04. 실제 운행은 강풍·기상으로 당일 중단될 수 있으므로 다음 날 아침 다시 확인합니다.",
        "query": "Seceda Cable Car Ortisei"
      },
      {
        "time": "17:00 이후",
        "name": "오르티세이 보행자 거리 · 숙소 스파",
        "lat": 46.5744,
        "lng": 11.6707,
        "kind": "휴식",
        "desc": "가벼운 산책 뒤 숙소 스파와 이른 저녁. 첫날 체력을 남겨 두는 일정입니다.",
        "tip": "부활절 월요일 저녁 식당은 예약 권장.",
        "query": "Ortisei pedestrian street"
      }
    ],
    "parking": [
      {
        "name": "오르티세이 숙소 주차 우선",
        "query": "Parcheggio Central Ortisei",
        "desc": "3박 동안 차량을 자주 쓰지 않도록 숙소 전용 주차를 최우선. 체크인 전 대안은 Parcheggio Central.",
        "url": ""
      }
    ]
  },
  {
    "date": "3.30",
    "dow": "화",
    "city": "돌로미티",
    "title": "세체다 · 이번 돌로미티의 1순위",
    "sub": "운행 확정 리프트 + 후기 좋은 대표 전망",
    "stay": "오르티세이 / 3박 중 2박",
    "move": "도보 + Ortisei–Furnes + Furnes–Seceda 왕복 · 차량 이동 없음",
    "note": "Seceda의 Ortisei–Furnes와 Furnes–Seceda는 공식적으로 2026-12-04~2027-04-04 운행 예정이라 3/30이 시즌 안에 들어갑니다. 지도 리뷰 4.8/5(약 1,900건), Tripadvisor 4.6/5(약 450건) 수준으로 후기도 강합니다(2026-09-21 확인). 3/31보다 시야가 좋은 날을 Seceda에 우선 배정하고 두 날을 서로 바꿔도 됩니다.",
    "spots": [
      {
        "time": "08:45",
        "name": "숙소 → 세체다 케이블카 하부역",
        "lat": 46.5779,
        "lng": 11.6748,
        "kind": "도보",
        "desc": "오르티세이 중심 숙소에서 걸어서 이동. 현장 운행 상태와 마지막 하행 시각을 확인한 뒤 왕복권을 구매합니다.",
        "tip": "운행기간은 확정됐지만 강풍·시야·정비에 따른 당일 중단 가능성은 남습니다.",
        "query": "Seceda Cable Car Ortisei"
      },
      {
        "time": "09:00–10:00",
        "name": "Ortisei–Furnes → Furnes–Seceda",
        "lat": 46.5912,
        "lng": 11.7015,
        "kind": "리프트",
        "desc": "공식 2026/27 운행기간이 확인된 두 구간만 이용해 정상 권역으로 올라갑니다.",
        "tip": "스키 이용객 동선과 분리해 보행자 탑승 안내를 따르고, 스키 슬로프에는 들어가지 않습니다.",
        "query": "Furnes Seceda cable car"
      },
      {
        "time": "10:00–13:30",
        "name": "세체다 파노라마 포인트",
        "lat": 46.6003,
        "lng": 11.7244,
        "kind": "전망",
        "desc": "Odle 능선과 돌로미티 파노라마를 보는 핵심 시간. 사진과 전망에 집중하고 눈 상태가 허용되는 개방 구역만 짧게 걷습니다.",
        "tip": "여름 하이킹 코스를 그대로 따라가지 않습니다. 방풍·방수 신발과 선글라스 준비.",
        "query": "Seceda Panorama Point"
      },
      {
        "time": "13:30–14:30",
        "name": "정상 권역 점심",
        "lat": 46.6000,
        "lng": 11.7200,
        "kind": "점심",
        "desc": "당일 영업 중인 산장에서 따뜻한 점심. 특정 산장을 고정하지 않고 현장 영업 여부를 우선합니다.",
        "tip": "혼잡하면 점심을 조금 일찍 먹고 하행 대기 시간을 줄입니다.",
        "query": "Seceda mountain hut"
      },
      {
        "time": "15:30 이후",
        "name": "오르티세이 귀환 · 스파",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "휴식",
        "desc": "하산 후 카페나 숙소 스파. 저녁 일정은 비워 둡니다.",
        "tip": "",
        "query": "Ortisei"
      }
    ]
  },
  {
    "date": "3.31",
    "dow": "수",
    "city": "돌로미티",
    "title": "알페 디 시우시 · 설원과 사솔룽고",
    "sub": "운행 확정된 Siusi 곤돌라로 안전하게 접근",
    "stay": "오르티세이 / 3박 중 3박",
    "move": "차량 약 20km·30분 편도 + Siusi–Alpe di Siusi 곤돌라 왕복",
    "note": "오르티세이 직행 Mont Sëuc 곤돌라는 현재 공개 페이지의 개별 2026/27 운행일 표기가 불완전해 일정에서 제외합니다. 대신 Siusi–Alpe di Siusi 곤돌라는 공식적으로 2026-12-05~2027-04-04 매일 08:00~18:00 운행 예정이라 3/31 이용이 확인됩니다. Alpe di Siusi는 Tripadvisor 약 400건의 리뷰가 있고 최근 겨울 산책 후기에서도 설원 풍경 평가가 좋습니다.",
    "spots": [
      {
        "time": "08:15",
        "name": "오르티세이 숙소 출발",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "차량",
        "desc": "숙소에서 Siusi의 Seiser Alm 곤돌라 하부역으로 이동. 약 20km, 계획상 30분 정도 잡습니다.",
        "tip": "산 위 Compatsch까지 직접 운전하지 않고 하부역에 주차합니다.",
        "query": "Seiser Alm Aerial Cableway valley station"
      },
      {
        "time": "08:50",
        "name": "Siusi–Alpe di Siusi 하부역 · 주차",
        "lat": 46.5416,
        "lng": 11.5688,
        "kind": "주차",
        "desc": "하부역의 무료 야외주차장 또는 유료 주차장을 이용합니다. 차량을 세운 뒤 곤돌라로 이동합니다.",
        "tip": "공식 안내상 하부역 무료 야외주차와 유료 주차장이 있습니다. 산 위 차량 진입은 09:00~17:00 제한되므로 올라가지 않습니다.",
        "query": "Seiser Alm Bahn Talstation parking"
      },
      {
        "time": "09:00–09:20",
        "name": "Siusi–Alpe di Siusi 곤돌라",
        "lat": 46.5416,
        "lng": 11.5688,
        "kind": "리프트",
        "desc": "2026/27 겨울 운행이 확정된 곤돌라를 타고 Compatsch로 올라갑니다.",
        "tip": "공식 시즌: 2026-12-05~2027-04-04, 매일 08:00~18:00. 기상에 따른 당일 변동은 출발 전 재확인.",
        "query": "Seiser Alm Aerial Cableway"
      },
      {
        "time": "09:30–13:30",
        "name": "알페 디 시우시 · Compatsch 설원 전망",
        "lat": 46.5407,
        "lng": 11.6171,
        "kind": "전망",
        "desc": "Schlern과 Sassolungo 방향 설원을 천천히 감상합니다. 운영·제설된 겨울 산책 구간만 짧게 걷고 멀리 확장하지 않습니다.",
        "tip": "목표는 긴 트레킹이 아니라 설원 풍경과 사진. 체력과 노면 상태를 보고 즉시 축소할 수 있게 합니다.",
        "query": "Compatsch Alpe di Siusi"
      },
      {
        "time": "13:30–14:30",
        "name": "Compatsch 점심",
        "lat": 46.5407,
        "lng": 11.6171,
        "kind": "점심",
        "desc": "Compatsch 권역에서 영업 중인 식당을 골라 점심. 리프트를 추가로 갈아타는 일정은 넣지 않습니다.",
        "tip": "추가 리프트는 2027 개별 운행 확인 전까지 일정에서 제외.",
        "query": "Compatsch restaurant"
      },
      {
        "time": "15:00–16:00",
        "name": "곤돌라 하산 → 오르티세이 복귀",
        "lat": 46.5416,
        "lng": 11.5688,
        "kind": "귀환",
        "desc": "혼잡 전 하산해 차로 오르티세이 숙소로 돌아옵니다. 마지막 밤은 스파와 저녁으로 마무리합니다.",
        "tip": "다음 날 베네치아 장거리 이동을 위해 짐은 저녁에 미리 정리합니다.",
        "query": "Ortisei"
      }
    ],
    "parking": [
      {
        "name": "Seiser Alm 곤돌라 하부역 주차",
        "query": "Seiser Alm Bahn Talstation parking",
        "desc": "공식 안내상 대형 무료 야외주차장과 유료 Walderer/하부역 주차장이 있습니다. 당일 주차 후 곤돌라 이용.",
        "url": "https://www.seiseralm.it/en/info-service/mobility/seiser-alm-aerial-cableway-winter.html"
      }
    ]
  },
  {
    "date": "4.1",
    "dow": "목",
    "city": "돌로미티 → 베네치아",
    "title": "설산에서 물의 도시로",
    "sub": "차는 섬 입구에, 여행은 배 위에서",
    "stay": "베네치아 본섬 · 산타루치아역/산폴로 인근 / 1박",
    "move": "약 300km · 운전 3.5~4.5시간 + 수상 이동",
    "note": "A22–A4 경유를 기본으로 두고 산악 고개 횡단은 제외. 본섬 숙박용 작은 가방을 준비하되 귀중품은 모두 소지합니다.",
    "spots": [
      {
        "time": "09:00",
        "name": "오르티세이 출발",
        "lat": 46.5753,
        "lng": 11.6714,
        "kind": "차량",
        "desc": "조식 후 출발. 중간 휴식 포함 오후 초반 주차장 도착 목표.",
        "tip": "",
        "query": "오르티세이 출발"
      },
      {
        "time": "13:30",
        "name": "트론케토 주차장",
        "lat": 45.4423,
        "lng": 12.3056,
        "kind": "주차",
        "desc": "차를 주차하고 바포레토 또는 피플무버 연계로 숙소 이동.",
        "tip": "24시간 이상 주차·수상버스 표 확인. 본섬 호텔 주소를 차량 내비 목적지로 넣지 않기.",
        "query": "트론케토 주차장"
      },
      {
        "time": "16:00",
        "name": "리알토 다리",
        "lat": 45.438,
        "lng": 12.3359,
        "kind": "산책",
        "desc": "체크인 후 대운하와 골목 산책. 바카로에서 치케티로 간단히 요기.",
        "tip": "",
        "query": "리알토 다리"
      },
      {
        "time": "17:30–19:30",
        "name": "산마르코 광장",
        "lat": 45.4342,
        "lng": 12.3388,
        "kind": "전망",
        "desc": "저녁빛의 광장과 해안 산책. 곤돌라는 날씨·대기·현장 요금을 보고 선택.",
        "tip": "2027년 입도료 시행일과 숙박객 면제 절차는 공식 공지 재확인.",
        "query": "산마르코 광장"
      }
    ],
    "parking": [
      {
        "name": "Tronchetto Parking · 베네치아",
        "query": "Tronchetto Parking Venezia",
        "desc": "공식 안내 24시간 운영, ZTL 밖. 바포레토·People Mover 연계. 4/1 입차~4/2 출차 기준 요금 확인.",
        "url": "https://www.tronchettoparking.it/it/"
      }
    ]
  },
  {
    "date": "4.2",
    "dow": "금",
    "city": "베네치아 → 시에나",
    "title": "운하에서 토스카나로",
    "sub": "볼로냐 숙박 없이 시에나로 바로",
    "stay": "시에나 · 성벽 밖 주차 가능한 숙소 / 2박 중 1박 · 토스카나 1/4박",
    "move": "운전 약 4~5시간 + Cantagallo Ovest 휴식 45~60분 + 본섬 출차 이동 (계획용 추정)",
    "note": "11:00~11:30 트론케토 출발 → 13:30 전후 Cantagallo Ovest에서 주유·점심 → 16:30~17:30 시에나 도착 목표. 볼로냐 시내 방문 없이 고속도로 휴게소만 이용. 정체·피로가 있으면 더 이른 휴게소에서 추가 휴식.",
    "parking": [
      {
        "name": "Santa Caterina · 기본 주차장",
        "query": "Parcheggio Santa Caterina Siena",
        "desc": "성벽 밖에 주차한 뒤 에스컬레이터로 구시가지 접근. 숙소 전용 주차가 있으면 우선 이용.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "name": "Il Campo · 대안",
        "query": "Parcheggio Il Campo Siena",
        "desc": "캄포 광장 방문 대안. 내비에 광장이 아니라 주차장명을 입력하고 ZTL 표지와 공식 진입 안내를 확인.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "name": "Cantagallo Ovest · 남행 휴게소",
        "query": "Area di Servizio Cantagallo Ovest A1 direzione Firenze",
        "desc": "A1 198.9km DirSUD. 고속도로 안에서 주유·식사·주차. 상용차 전용 주유구역 대신 실제 차량에 맞는 주유기 확인.",
        "url": "https://www.q8truck.com/pt/stations/cantagallo-ovest-boa1-q8-it3888"
      }
    ],
    "spots": [
      {
        "time": "08:00–09:00",
        "name": "산마르코 광장",
        "lat": 45.4342,
        "lng": 12.3388,
        "kind": "산책",
        "desc": "아침 광장과 조식. 큰 짐은 체크아웃 동선에 맞춰 정리.",
        "tip": "",
        "query": "산마르코 광장",
        "url": ""
      },
      {
        "time": "09:00–10:00",
        "name": "산마르코 대성당",
        "lat": 45.4346,
        "lng": 12.3397,
        "kind": "예약",
        "desc": "공식 예약 가능 시간에 맞춰 관람. 오전 예약이 불가능하면 광장 산책으로 대체.",
        "tip": "예약 미확정. 10시 무렵 주차장으로 이동 시작.",
        "query": "산마르코 대성당",
        "url": ""
      },
      {
        "time": "11:00–11:30",
        "name": "Tronchetto Parking · 차량 회수",
        "lat": 45.4423,
        "lng": 12.3056,
        "kind": "주차",
        "desc": "차량 회수 후 A13–볼로냐–A1 남행으로 시에나 이동. 약 2~2.5시간 뒤 Cantagallo Ovest에서 주유·점심·휴식.",
        "tip": "본섬에서 주차장 이동 시간을 확보.",
        "query": "Tronchetto Parking Venezia",
        "url": "https://www.tronchettoparking.it/it/"
      },
      {
        "time": "13:30–14:30 예상",
        "name": "Cantagallo Ovest · 주유와 점심",
        "lat": 44.455051,
        "lng": 11.280162,
        "kind": "휴게소",
        "desc": "볼로냐 남쪽 A1 198.9km, Firenze/Roma 남행 방향 휴게소. 주유·화장실·점심을 한 번에 해결하고 45~60분 휴식. Autogrill의 La Fucina 셀프서비스 식당 또는 Bistrot Bakery 피자·포카치아 중 선택.",
        "tip": "반대편 Est와 혼동하지 말고 A1 남행 Cantagallo Ovest 표지로 진입. 주유소 공식 자료는 디젤 등 상용차 설비를 안내하므로 렌터카 연료에 맞는 승용차 주유기·결제 가능 여부를 현장에서 확인. 식당 영업·메뉴는 2027년 재확인.",
        "query": "Area di Servizio Cantagallo Ovest A1 direzione Firenze",
        "url": "https://www.autogrill.it/locale/cantagallo/"
      },
      {
        "time": "16:30–17:30 예상",
        "name": "Santa Caterina · 시에나 주차",
        "lat": 43.3162,
        "lng": 11.326,
        "kind": "주차",
        "desc": "숙소 주차 또는 Santa Caterina 이용 후 체크인. 도착 시간은 교통 상황에 따라 조정.",
        "tip": "지도 핀은 대표 위치. 아래 주차장명 길찾기와 현장 진입 표지를 우선.",
        "query": "Parcheggio Santa Caterina Siena",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "time": "18:00–20:00",
        "name": "캄포 광장",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "짧은 골목 산책과 피치 파스타 저녁. 장거리 이동 후 충분히 휴식.",
        "tip": "",
        "query": "캄포 광장",
        "url": ""
      }
    ]
  },
  {
    "date": "4.3",
    "dow": "토",
    "city": "시에나",
    "title": "시에나에 온전히 머무는 하루",
    "sub": "대성당·캄포 광장·느린 골목 산책",
    "stay": "시에나 / 2박 중 2박 · 토스카나 2/4박",
    "move": "도보 중심 · 차량은 숙소 또는 같은 주차장에",
    "note": "추가한 하루는 시에나를 여유 있게 보는 날로 사용합니다. 숙박 중 차를 옮기지 않고 다음 날 발도르차로 출발. 주차요금은 실제 체류시간으로 확인.",
    "parking": [
      {
        "name": "Santa Caterina · 기본 주차장",
        "query": "Parcheggio Santa Caterina Siena",
        "desc": "성벽 밖에 주차한 뒤 에스컬레이터로 구시가지 접근. 숙소 전용 주차가 있으면 우선 이용.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      },
      {
        "name": "Il Campo · 대안",
        "query": "Parcheggio Il Campo Siena",
        "desc": "캄포 광장 방문 대안. 내비에 광장이 아니라 주차장명을 입력하고 ZTL 표지와 공식 진입 안내를 확인.",
        "url": "https://sigericospa.it/parcheggi-in-struttura/"
      }
    ],
    "spots": [
      {
        "time": "09:30–11:30",
        "name": "시에나 대성당",
        "lat": 43.3177,
        "lng": 11.3289,
        "kind": "예약",
        "desc": "대성당과 통합권 포함 시설을 취향에 맞춰 관람. 예약 가능 시간 확인.",
        "tip": "",
        "query": "시에나 대성당",
        "url": ""
      },
      {
        "time": "12:00–14:00",
        "name": "캄포 광장",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "점심 후 광장에 앉아 쉬기. 만자 탑은 체력과 입장 상황에 따라 선택.",
        "tip": "",
        "query": "캄포 광장",
        "url": ""
      },
      {
        "time": "14:30–16:00",
        "name": "산 도메니코 성당",
        "lat": 43.3204,
        "lng": 11.3272,
        "kind": "산책",
        "desc": "구시가지 골목과 성당 주변 전망 산책. 피곤하면 카페·숙소 휴식.",
        "tip": "",
        "query": "산 도메니코 성당",
        "url": ""
      },
      {
        "time": "18:30",
        "name": "시에나 구시가지",
        "lat": 43.3186,
        "lng": 11.3317,
        "kind": "식사",
        "desc": "도보로 저녁 식사. 다음 날 농가 이동을 위해 짐 정리.",
        "tip": "",
        "query": "시에나 구시가지",
        "url": ""
      }
    ]
  },
  {
    "date": "4.4",
    "dow": "일",
    "city": "시에나 → 발도르차 → 농가",
    "title": "와인 마을을 지나 농가로",
    "sub": "몬탈치노·바뇨 비뇨니·피엔차",
    "stay": "Fattoria del Colle · Trequanda / 추천 숙소 2박 중 1박 · 토스카나 3/4박 · 예약 미확정",
    "move": "시에나 → 몬탈치노 → 바뇨 비뇨니 → 피엔차 → 트레콴다 · 총 운전 약 2.5~3시간",
    "note": "서쪽 몬탈치노에서 동쪽 피엔차를 거쳐 농가로 이동합니다. 이날은 마을 관광, 와인 시음은 다음 날 숙소 현장 투어에 배치. 모든 시간은 주차·교통에 따라 조정.",
    "parking": [
      {
        "name": "몬탈치노 · Fortezza 주변 주차 후보",
        "query": "Parcheggio Fortezza Montalcino",
        "desc": "성곽 바깥 주차 후보. 정확한 입구·공사·요금은 현지 표지에서 확인. 지도 검색 결과에서 주차장 선택.",
        "url": ""
      },
      {
        "name": "바뇨 비뇨니 · 마을 입구 공영주차",
        "query": "Parcheggio Bagno Vignoni",
        "desc": "중앙 수조로 진입하지 말고 마을 입구 주차 표지를 따르기. 현지 운영 확인.",
        "url": ""
      },
      {
        "name": "피엔차 · 성벽 밖 공영주차",
        "query": "Parcheggio Pienza",
        "desc": "역사 중심부를 목적지로 운전하지 말고 성벽 밖 주차장을 선택. 일요일 혼잡 시 여유 시간 확보.",
        "url": ""
      }
    ],
    "spots": [
      {
        "time": "09:00",
        "name": "Santa Caterina · 시에나 출발",
        "lat": 43.3162,
        "lng": 11.326,
        "kind": "차량",
        "desc": "체크아웃 후 차량 회수. 짐은 외부에서 보이지 않게 정리.",
        "tip": "",
        "query": "Parcheggio Santa Caterina Siena",
        "url": ""
      },
      {
        "time": "10:15–11:45",
        "name": "몬탈치노 성곽·구시가지",
        "lat": 43.0568,
        "lng": 11.4892,
        "kind": "마을",
        "desc": "브루넬로의 마을에서 성곽과 전망 산책. 운전자는 시음하지 않고 병 구매만.",
        "tip": "성곽 바깥 주차 후 도보.",
        "query": "Fortezza Montalcino",
        "url": "https://www.visittuscany.com/en/towns-and-villages/montalcino/"
      },
      {
        "time": "12:15–13:15",
        "name": "바뇨 비뇨니",
        "lat": 43.0287,
        "lng": 11.6183,
        "kind": "마을",
        "desc": "온천 광장 산책과 가벼운 점심. 중앙 수조는 목욕 장소가 아닙니다.",
        "tip": "",
        "query": "바뇨 비뇨니",
        "url": ""
      },
      {
        "time": "14:00–15:30",
        "name": "피엔차",
        "lat": 43.0775,
        "lng": 11.6797,
        "kind": "마을",
        "desc": "전망 산책로와 골목, 페코리노 치즈 가게. 농가 체크인을 위해 오후 일찍 출발.",
        "tip": "",
        "query": "피엔차",
        "url": ""
      },
      {
        "time": "16:30",
        "name": "Fattoria del Colle · 체크인",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "숙박",
        "desc": "추천 농가에서 2박. 트레콴다 권역 핀은 대략 위치이며 길찾기는 숙소명으로 연결됩니다.",
        "tip": "예약 시 전용 주차·진입로·체크인 마감·4/4와 4/5 저녁 운영 확인.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      },
      {
        "time": "19:00",
        "name": "Fattoria del Colle · 농가 저녁",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "식사",
        "desc": "식당 운영과 예약 확정 시 농가에서 저녁. 운영하지 않으면 시음 전 이동 가능한 식당 또는 숙소 식사 대안을 준비.",
        "tip": "숙소·식사·투어 모두 예약 미확정.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      }
    ]
  },
  {
    "date": "4.5",
    "dow": "월",
    "city": "토스카나 · 농가",
    "title": "포도밭과 와인, 느린 하루",
    "sub": "농가 산책·저장고 투어·숙소 저녁",
    "stay": "Fattoria del Colle · Trequanda / 추천 숙소 2박 중 2박 · 토스카나 4/4박 · 예약 미확정",
    "move": "숙소 중심 · 선택 시 트레콴다 마을 짧은 왕복",
    "note": "둘 다 와인을 즐길 수 있도록 오후 투어부터 차를 사용하지 않습니다. 숙박과 투어는 별도 확정하며, 몬탈치노의 Casato Prime Donne가 아닌 Fattoria del Colle 현장 프로그램을 선택하세요.",
    "spots": [
      {
        "time": "09:00–11:00",
        "name": "Fattoria del Colle · 농가 산책",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "휴식",
        "desc": "조식 후 농가 부지의 개방된 산책 구역에서 휴식. 4월에는 포도 수확 체험을 전제로 하지 않습니다.",
        "tip": "정확한 위치·주차는 숙소 안내 기준.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      },
      {
        "time": "11:30–13:30",
        "name": "트레콴다 · 선택 산책",
        "lat": 43.1884,
        "lng": 11.6688,
        "kind": "마을",
        "desc": "컨디션이 좋으면 가까운 마을에서 점심. 차량 이동을 마치고 농가로 돌아온 뒤 시음.",
        "tip": "구시가지 바깥 주차 표지 이용. 쉬고 싶으면 농가에 머물기.",
        "query": "Trequanda",
        "url": ""
      },
      {
        "time": "15:30–16:30",
        "name": "Fattoria del Colle · 와인 투어",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "예약",
        "desc": "빌라·정원·역사적 저장고와 와인 3종 시음. 공식 현행 안내 약 1시간, 1인 €29.50; 2027년 가격·월요일 운영·영어 진행은 예약 시 재확인.",
        "tip": "투어 후 운전 일정 없음. 숙소 안에서 진행되는 프로그램으로 예약.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/dettaglio/sale-della-villa-del-cinquecento-cappella-giardini-antichi-cantina-storica-con-degustazione-itinerante/"
      },
      {
        "time": "17:00–저녁",
        "name": "Fattoria del Colle · 휴식과 저녁",
        "lat": 43.18,
        "lng": 11.667,
        "kind": "휴식",
        "desc": "농가에서 느긋한 저녁. 다음 날 피렌체로 이동할 준비.",
        "tip": "당일 식당 운영과 저녁 예약 필수 확인.",
        "query": "Fattoria del Colle · Trequanda",
        "url": "https://www.cinellicolombini.it/turismo/camere/"
      }
    ]
  },
  {
    "date": "4.6",
    "dow": "화",
    "city": "토스카나 → 피렌체",
    "title": "농가에서 르네상스의 도시로",
    "sub": "오전 이동 후 기존 피렌체 일정",
    "stay": "피렌체 · ZTL 밖 주차 가능한 숙소 / 2박 중 1박",
    "move": "트레콴다 → A1 → Villa Costanza · 운전 약 1.5~2시간 + 트램·주차",
    "note": "농가에서 조식 후 09시 무렵 출발. A1 경유로 Villa Costanza 주차장에 진입하고 트램 T1으로 도심 이동. 피렌체 2박과 우피치 관람 날짜는 유지.",
    "spots": [
      {
        "time": "11:00",
        "name": "Villa Costanza 주차장",
        "lat": 43.7541,
        "lng": 11.1734,
        "kind": "주차",
        "desc": "A1에서 전용 출구로 진입해 주차. 트램 T1으로 피렌체 도심 이동 후 짐 보관.",
        "tip": "2027년 요금·차량 조건 재확인. 호텔 전용 주차를 선택하면 실제 출발·회수 장소도 함께 변경.",
        "query": "Parcheggio Villa Costanza Scandicci",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      },
      {
        "time": "13:00–15:00",
        "name": "피렌체 두오모",
        "lat": 43.7731,
        "lng": 11.256,
        "kind": "관람",
        "desc": "점심 후 대성당 광장과 주변 골목 산책. 돔 등반은 원하면 예약 시간에 맞춰 추가.",
        "tip": "돔 등반은 체력·계단을 고려. 이 기본안은 외관 중심.",
        "query": "피렌체 두오모"
      },
      {
        "time": "15:30",
        "name": "시뇨리아 광장",
        "lat": 43.7696,
        "lng": 11.2558,
        "kind": "산책",
        "desc": "야외 조각을 보고 젤라토 휴식.",
        "tip": "",
        "query": "시뇨리아 광장"
      },
      {
        "time": "17:00–19:00",
        "name": "베키오 다리",
        "lat": 43.768,
        "lng": 11.2531,
        "kind": "전망",
        "desc": "아르노 강변을 걷고 올트라르노에서 저녁.",
        "tip": "",
        "query": "베키오 다리"
      }
    ],
    "parking": [
      {
        "name": "Villa Costanza · 피렌체 2박",
        "query": "Parcheggio Villa Costanza Scandicci",
        "desc": "A1 고속도로에서만 차량 진입. 트램 T1으로 SMN역까지 공식 안내 약 22분. 최대 30일 연속 주차 가능, 트램 별도 결제. 2027년 요금 재확인.",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      }
    ]
  },
  {
    "date": "4.7",
    "dow": "수",
    "city": "피렌체",
    "title": "예술 한 번, 노을 한 번",
    "sub": "우피치와 도시를 내려다보는 저녁",
    "stay": "피렌체 / 2박 중 2박",
    "move": "도보 중심 · 전망대는 버스·택시 가능",
    "note": "대형 미술관은 하루 하나만. 우피치를 수요일 오전에 배치했습니다. 아래 시간은 예약 목표이며 확정 입장권이 아닙니다.",
    "spots": [
      {
        "time": "09:00–12:00",
        "name": "우피치 미술관",
        "lat": 43.7678,
        "lng": 11.2553,
        "kind": "예약",
        "desc": "보티첼리와 르네상스 작품 중심으로 약 3시간 관람. 공식 시간 지정 예약 추천.",
        "tip": "",
        "query": "우피치 미술관"
      },
      {
        "time": "12:30–15:00",
        "name": "산토 스피리토 광장",
        "lat": 43.7665,
        "lng": 11.2478,
        "kind": "식사",
        "desc": "올트라르노에서 점심 후 공방 골목. 중간에 호텔에서 쉬어도 좋습니다.",
        "tip": "",
        "query": "산토 스피리토 광장"
      },
      {
        "time": "17:30–19:30",
        "name": "미켈란젤로 광장",
        "lat": 43.7629,
        "lng": 11.2651,
        "kind": "전망",
        "desc": "해 지기 전 도착해 피렌체 전경과 노을 사진. 실제 일몰·날씨에 맞춰 시간을 조정.",
        "tip": "오르막은 버스·택시로 이동 가능. 차는 숙소에 두기.",
        "query": "미켈란젤로 광장"
      },
      {
        "time": "20:00",
        "name": "산토 스피리토 일대",
        "lat": 43.7665,
        "lng": 11.2478,
        "kind": "식사",
        "desc": "여행 후반의 기념 저녁. 비스테카는 2인 양·가격을 먼저 확인 후 주문.",
        "tip": "",
        "query": "산토 스피리토 일대"
      }
    ],
    "parking": [
      {
        "name": "Villa Costanza · 피렌체 2박",
        "query": "Parcheggio Villa Costanza Scandicci",
        "desc": "A1 고속도로에서만 차량 진입. 트램 T1으로 SMN역까지 공식 안내 약 22분. 최대 30일 연속 주차 가능, 트램 별도 결제. 2027년 요금 재확인.",
        "url": "https://parcheggiovillacostanza.it/en/faq/"
      }
    ]
  },
  {
    "date": "4.8",
    "dow": "목",
    "city": "피렌체 → 파르마",
    "title": "미식 도시에서 마지막 밤",
    "sub": "산을 넘어 에밀리아로",
    "stay": "파르마 · 구시가지 외곽 주차 가능한 숙소 / 1박",
    "move": "약 190km · 운전 2.5~3시간",
    "note": "아침 치즈 공장 투어는 오늘 이동과 맞추기 어렵습니다. 이번 기본안에서는 파르마 도심과 여유로운 마지막 저녁을 선택합니다.",
    "spots": [
      {
        "time": "09:30",
        "name": "Villa Costanza · 차량 회수",
        "lat": 43.7541,
        "lng": 11.1734,
        "kind": "차량",
        "desc": "실제 주차 장소에서 차를 찾아 파르마로 출발. 고속도로 휴식 1회.",
        "tip": "",
        "query": "Villa Costanza · 차량 회수"
      },
      {
        "time": "13:00",
        "name": "Parcheggio Toschi · 파르마",
        "lat": 44.8076,
        "lng": 10.3271,
        "kind": "주차",
        "desc": "주차·체크인 후 점심. 진입 경로는 주차장 공식 안내 확인.",
        "tip": "",
        "query": "Parcheggio Toschi Parma"
      },
      {
        "time": "14:30–16:00",
        "name": "파르마 대성당",
        "lat": 44.8035,
        "lng": 10.3302,
        "kind": "관람",
        "desc": "대성당과 세례당 광장 산책. 내부는 개관·입장권 확인 후 관람.",
        "tip": "",
        "query": "파르마 대성당"
      },
      {
        "time": "16:30",
        "name": "팔라초 델라 필로타",
        "lat": 44.8065,
        "lng": 10.3263,
        "kind": "산책",
        "desc": "광장 산책 또는 운영 시 테아트로 파르네세 관람 중 선택.",
        "tip": "",
        "query": "팔라초 델라 필로타"
      },
      {
        "time": "19:00",
        "name": "가리발디 광장 · 파르마",
        "lat": 44.8012,
        "lng": 10.328,
        "kind": "식사",
        "desc": "프로슈토 디 파르마, 파르미자노 레자노, 토르텔리로 마지막 만찬. 숙소까지 도보.",
        "tip": "",
        "query": "가리발디 광장 Parma"
      }
    ],
    "parking": [
      {
        "name": "Toschi · 파르마",
        "query": "Parcheggio Toschi Parma",
        "desc": "기존 주차 후보 유지. 도심 관광·1박 주차는 숙소 전용 주차와 비교하고 진입로·운영을 출발 전 확인.",
        "url": ""
      }
    ]
  },
  {
    "date": "4.9",
    "dow": "금",
    "city": "파르마 → 밀라노 반납 → MXP",
    "title": "차를 돌려주고 공항으로",
    "sub": "16시 밀라노 중앙역 반납 · 기차로 공항 이동",
    "stay": "기내 · 한국 도착 날짜는 항공권 확인",
    "move": "파르마 → 밀라노 중앙역 권역 운전 약 2~2.5시간 + 주유·반납 / 이후 공항철도 약 1시간 + 도보·대기",
    "note": "Hertz 예약서 기준 4/9(금) 16:00 밀라노 중앙역 영업소 반납. 반납 주소는 Via Alfredo Cappellini 21, 5층이며 수령 주소 10번지와 다릅니다. 공항 반납이 아닙니다. 예약서 일반 금요일 영업시간 08:00–20:00. KE928 MXP 22:00 출발 기준 19시 전 출발 터미널 도착 목표.",
    "spots": [
      {
        "time": "09:00–10:30",
        "name": "파르마 · 아침과 짐 정리",
        "lat": 44.8012,
        "lng": 10.328,
        "kind": "식사",
        "desc": "조식 후 간단한 식품 쇼핑과 체크아웃. 점심은 일찍 먹거나 이동 중 간단히 해결.",
        "tip": "16시 시내 반납에 맞춰 오전 관광을 짧게.",
        "query": "Piazza Garibaldi Parma",
        "url": ""
      },
      {
        "time": "11:30 출발 목표",
        "name": "Parcheggio Toschi · 밀라노로 출발",
        "lat": 44.8076,
        "lng": 10.3271,
        "kind": "차량",
        "desc": "파르마에서 밀라노 중앙역 권역으로 출발. 공항으로 바로 운전하지 않습니다.",
        "tip": "금요일 시내 정체·주유·주차장 진입을 위해 여유 확보.",
        "query": "Parcheggio Toschi Parma",
        "url": ""
      },
      {
        "time": "14:00–15:00",
        "name": "밀라노 · 반납 전 주유",
        "lat": 45.4804,
        "lng": 9.2036,
        "kind": "주유",
        "desc": "Hertz 인수 시 확인한 연료 조건에 맞춰 반납점 접근 전 주유. 실제 진입 경로의 영업 중 주유소를 선택하고 영수증 보관.",
        "tip": "지도 핀은 반납 권역의 대표 위치이며 특정 주유소가 아닙니다. 내비에서 차량 연료에 맞는 주유소 확인.",
        "query": "distributore carburante vicino Via Alfredo Cappellini Milano",
        "url": ""
      },
      {
        "time": "15:30 도착 / 16:00 예약",
        "name": "Hertz · 밀라노 중앙역 차량 반납",
        "lat": 45.4804,
        "lng": 9.2036,
        "kind": "예약 완료",
        "desc": "Via Alfredo Cappellini 21, Milano 20124 · 5층 반납 구역. Hertz 반납 표지를 따라 진입하고 차량 사진·반납 확인서를 확보.",
        "tip": "수령점 Via Cappellini 10으로 가지 않기. 정확한 차량 입구·진입 경로는 수령할 때 확인. 16시까지 인계를 마치도록 일찍 도착.",
        "query": "Hertz return Via Alfredo Cappellini 21 Milano",
        "url": ""
      },
      {
        "time": "16:30–17:30 출발편 목표",
        "name": "밀라노 중앙역 · Malpensa Express",
        "lat": 45.4857,
        "lng": 9.2044,
        "kind": "공항철도",
        "desc": "반납 후 도보 또는 택시로 Milano Centrale 이동. Malpensa Express로 항공권의 출발 터미널까지 이동. 도보·승강장 이동·대기를 포함해 여유 확보.",
        "tip": "현재 공식 안내는 중앙역 출발 약 30분 간격. 2027년 4/9 실제 시간표·파업·터미널 확인 후 승차권 구매.",
        "query": "Milano Centrale",
        "url": "https://www.malpensaexpress.it/en/lines-and-timetable/lines-and-timetables/timetable/"
      },
      {
        "time": "18:00–19:00 도착 / 22:00 출발",
        "name": "밀라노 말펜사 공항 · 출국",
        "lat": 45.6301,
        "lng": 8.7231,
        "kind": "공항",
        "desc": "19시 전 출발 터미널 도착 목표. 체크인·수하물·보안 검색 후 KE928 탑승.",
        "tip": "항공권의 출발 시각과 터미널, 항공사 도착 권고를 최종 확인.",
        "query": "Milano Malpensa Airport",
        "url": ""
      }
    ]
  }
];
